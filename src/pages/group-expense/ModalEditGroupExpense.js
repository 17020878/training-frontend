...
export default function ModalEditGroupExpense(props) {
    const {updateOrganization,listGroupExpenseTree, isUpdate, openModalEdit, handleCloseModalEdit, isRefresh,setIsRefresh} = props;
    ...
    const createGroupExpenseApi = (body) => {
        return apiGroupExpense.createGroupExpense(body)
    }
    const updateGroupExpenseApi = (body) => {
        return apiGroupExpense.updateGroupExpense(body)
    }
            ...
            <Dialog className={"modal"} open={openModalEdit} onClose={handleCloseModalEdit}>
                ...
                <DialogTitle>
                    <div className={'vmp-tittle'}>
                        {
                            isUpdate?"Sửa nhóm chi phí":"Thêm nhóm chi phí"
                        }
                    </div>
                    <IconButton
                        aria-label="close"
                                ...
                                <Box sx={{flexGrow: 1}} className={'form-content'}>
                                    <Grid container spacing={4}>
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Nhóm chi phí<span
                                        ...
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Mã nhóm chi phí<span
                                                className={'error-message'}>*</span></div>
                                            <TextField
                                                size={"small"}
                                                id='code'
                                        ...
                                        </Grid>
                                        {
                                            updateOrganization.id==0?'':<Grid item xs={6} md={12}>
                                                <div className={'label-input'}>Nhóm chi phí cha<span
                                                    className={'error-message'}>*</span></div>
                                                <TreeSelect
