...
export default function ManageLecturer() {
...
//=====================================================================================================
    useEffect(() => {
        getConfigTableApi(getNameToId(tableName, 5)).then(r => {
...
//=====================================================================================================
    const addLecturerBtn = () => {
        navigate('/lecturer/create')
    }
    const updateLecturerBtn = (e) => {
        navigate(`/lecturer/update?id=${e}`)
    }
//=====================================================================================================
    const deleteLecturerApi = (data) => {
        return apiLecturer.deleteLecturer(data);
    }
    const searchLecturerApi = (data) => {
        return apiLecturer.searchLecturer(data);
    }
        ...
        <div className={'main-content'}>
            ...
            <div className={'main-content-body'}>
                <div className={'main-content-body-title flexGroup2'}>
                    <h4>Tìm kiếm</h4>
                    <Button onClick={addLecturerBtn} variant="outlined" startIcon={<AddIcon/>}>Thêm</Button>
                </div>
                <Divider light/>
                    ...
                    <div className={'form-control-search-header'}>
                        <div className={'flexGroup3'}>
                            <div className={'form-control-search-item'}>
                                <div className={'label-input'}>Tên giảng viên</div>
                                <TextField
                                    size={"small"}
                            ...
                            <Menu
                                ...
                                <SettingColumnTable columns={columns}
                                                    nameTable={getNameToId(tableName,5)}
                                ...
                                ? <Table stickyHeader className={"table-custom"}>
                                    <TableHead className={'super-app-theme--header'}>
                                        <TableRow>
                                            ...
                                            {/*<TableCell style={{minWidth: 150}}>Chuyên môn</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Số điện thoại</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Email</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Chức vụ</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Nơi công tác</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Kinh nghiệm</TableCell>*/}
                                            <TableCell className={'action-header'} style={{minWidth: 150}}
                                        ...
                                        {listResult.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item, i) => (
                                            <>
                                                <TableRow hover role="checkbox" tabIndex={-1}>
                                                    ...
                                                    {/*<TableCell rowSpan={1}>{getDateTimeFromTimestamp(item.birdOfDate)}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.expertise}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.phone}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.email}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.position}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.workPlace}</TableCell>*/}
                                                    {/*<TableCell rowSpan={1}>{item.experience}</TableCell>*/}
                                                    <TableCell className={'action-header filter-table'} rowSpan={1} align="center">
                                                        <div className='icon-action'>
                                                            <Tooltip title="Cập nhật"
                                                                     onClick={() => updateLecturerBtn(item.id)}>
                                                                <div><UpdateIcon/></div>
                                                            </Tooltip>
                                                            <Tooltip className={'deleteButton'} title="Xóa"
                                                                     onClick={() => deleteLecturerBtn(item.id)}>
                                                                <div><DeleteIcon/></div>
                                                            </Tooltip>
                                                        </div>
                                                    </TableCell>
