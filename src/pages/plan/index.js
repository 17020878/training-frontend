...
import Utils, {
    buildInputTree,
    buildTreeAsset,
    convertArr, formatVND, getDateTimeFromTimestamp,
    getNameToId,
    getNodeType,
    sortTreeData
...
//=====================================================================================================
    useEffect(() => {
        getConfigTableApi(getNameToId(tableName, 0)).then(r => {
...
//=====================================================================================================
    ...
    const deletePlanBtn = (e) => {
        setOpenModalDel(true)
        setPlanId(e)
    }
    const submitDelete = () => {
        if(planId){
            deletePlanApi(planId).then( r => {
...
//=====================================================================================================
    ...
    const updatePlanBtn = (e) => {
        navigate(`/plan/update?id=${e}`)
    }
//=====================================================================================================
    const deletePlanApi = (data) => {
        return apiPlan.deletePlan(data);
    }
    const searchPlanApi = (data) => {
        return apiPlan.searchPlan(data);
    }
                    ...
                    <div className={'form-control-search-header'}>
                        <div className={'flexGroup3'}>
                            <div className={'form-control-search-item'}>
                                <div className={'label-input'}>Tên kế hoạch</div>
                                <TextField
                                    size={"small"}
                            ...
                            <Menu
                                ...
                                <SettingColumnTable columns={columns}
                                                    nameTable={getNameToId(tableName, 0)}
                                    ...
                                    <TableHead className={'super-app-theme--header'}>
                                        <TableRow>
                                            ...
                                            {columns.map((column, columnIndex) => {
                                                if (column.visible) {
                                                    return <TableCell
                                                        style={{minWidth: 200}}>{column.name}</TableCell>
                                                }
                                            })}
                                            {/*<TableCell style={{minWidth: 150}}>Tên kế hoạch</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Mã kế hoạch</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 80}}>Năm kế hoạch</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 250}}>Khối</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 250}}>Đơn vị</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 150}}>Loại khóa đào tạo</TableCell>*/}
                                            {/*<TableCell style={{minWidth: 250}}>Ghi chú</TableCell>*/}
                                            <TableCell className={'action-header'} style={{minWidth: 50}}
                                                       align="center">Thao
                                                tác</TableCell>
                                        </TableRow>
                                    </TableHead>
                                        ...
                                        {listResult.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item, i) => (
                                            <>
                                                <TableRow hover role="checkbox" tabIndex={-1}>
                                                    ...
                                                    {columns.map((column, columnIndex) => {
                                                        if (column.visible) {
                                                            ...
                                                            if (column.code === 'lecturers'
                                                                ...
                                                                return (
                                                                    <TableCell
                                                                        rowSpan={1}>{new Date(item[column.code]).getFullYear()}</TableCell>
                                                                );
                                                            } else {
                                                                return (
                                                                    <TableCell key={columnIndex}
                                                    ...
                                                    {/*<TableCell rowSpan={1}>{new Date(item.planTime).getFullYear()}</TableCell>*/}
                                                    {/*<TableCell*/}
                                                    {/*    rowSpan={1}>{item.blockOrganization.name}</TableCell>*/}
                                                    {/*<TableCell*/}
                                                    {/*    rowSpan={1}>{item.unitOrganization.name}</TableCell>*/}
                                                    {/*<TableCell*/}
                                                    {/*    rowSpan={1}>{item.trainingType.name}</TableCell>*/}
