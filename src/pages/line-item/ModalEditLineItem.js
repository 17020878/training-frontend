...
export default function ModalEditLineItem(props) {
    const {updateOrganization,listLineItemTree, isUpdate, openModalEdit, handleCloseModalEdit, isRefresh,setIsRefresh} = props;
    ...
    const createGroupExpenseApi = (body) => {
        return apiLineItem.createLineItem(body)
    }
    const updateGroupExpenseApi = (body) => {
        return apiLineItem.updateLineItem(body)
    }
            ...
            <Dialog className={"modal"} open={openModalEdit} onClose={handleCloseModalEdit}>
                ...
                <DialogTitle>
                    <div className={'vmp-tittle'}>
                        {
                            isUpdate?"Sửa khoản mục":"Thêm khoản mục"
                        }
                    </div>
                    <IconButton
                        aria-label="close"
                    ...
                    {props => {
                        ...
                        return (
                            <Form onSubmit={handleSubmit}>
                                <Box sx={{flexGrow: 1}} className={'form-content'} style={{padding:'10px'}}>
                                    <Grid container spacing={4}>
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Tên khoản mục<span
                                        ...
                                        <Grid item xs={6} md={12}>
                                            <div className={'label-input'}>Mã khoản mục<span
                                                className={'error-message'}>*</span></div>
                                            <TextField
                                                size={"small"}
                                                id='code'
                                        ...
                                        </Grid>
                                        {
                                            updateOrganization.id==0?'':<Grid item xs={6} md={12}>
                                                <div className={'label-input'}>Khoản mục cha<span
                                                    className={'error-message'}>*</span></div>
                                                <TreeSelect
                                                    ...
                                                    placeholder="Tất cả khoản mục"></TreeSelect>
                                            </Grid>
                                        }
